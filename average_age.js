...
function getAverageMenAge(people) {
  ...
  people.forEach(element => {
    if (element.sex == "m") {
      let age = element.died - element.born;
      counter++;
      middleAge += age;
    }
  });
  return middleAge / counter;
}
...
function getAverageDiff(people) {
  ...
  for (let i = 0; i < people.length; i++) {
    for (let j = 0; j < people.length; j++) {
      if (people[i].name === people[j].mother) {
        let mother = people[i].died - people[i].born;
        let child = people[j].died - people[j].born;
        let diff = Math.abs(mother - child);
        counter++;
        averageDiff += diff;
        console.log(diff);
      }
  ...
  return averageDiff / counter;
}
let actualMen = getAverageMenAge(ANCESTRY_FILE);
console.log(actualMen);
let actualWomen = getAverageWomenAge(ANCESTRY_FILE);
console.log(actualWomen);
let averageDiff = getAverageDiff(ANCESTRY_FILE);
